...
public class TestUtils {
    ...
    /**
     * Validates the Kavalue and Clay record. TODO: understand why it's done this way.
     *
     * @param record
     */
    public static void validateKavalieAndClayRecord(final Record record) {
        assertEquals("leader", "00759cam a2200229 a 4500", record.getLeader().marshal());
        final Iterator<VariableField> it = record.getVariableFields().iterator();
        ...
        assertDataFieldEquals(it.next(), "245", '1', '4', "a", "The amazing adventures of Kavalier and Clay :", "b",
        ...
        assertDataFieldEquals(it.next(), "650", ' ', '0', "a", "Comic books, strips, etc.", "x", "Authorship", "v",
                "Fiction.");
     ...
     * @param record
     */
    public static void validateSummerlandRecord(final Record record) {
        ...
        assertDataFieldEquals(it.next(), "260", ' ', ' ', "a", "New York :", "b",
    ...
    /**
     * Validates the Freewheeling Bob Dylan record. Not quite sure why it's done this way (TODO: understand).
     *
     * @param record
     */
    public static void validateFreewheelingBobDylanRecord(final Record record) {
        ...
        assertDataFieldEquals(it.next(), "906", ' ', ' ', "a", "7", "b", "cbc", "c", "copycat", "d", "4", "e",
                "ncip", "f", "19", "g", "y-soundrec");
        ...
        assertDataFieldEquals(it.next(), "245", '1', '4', "a", "The freewheelin' Bob Dylan", "h",
                "[sound recording].");
        assertDataFieldEquals(it.next(), "260", ' ', ' ', "a", "[New York, N.Y.] :", "b", "Columbia,", "c", "[1963]");
        assertDataFieldEquals(it.next(), "300", ' ', ' ', "a", "1 sound disc :", "b",
                "analog, 33 1/3 rpm, stereo. ;", "c", "12 in.");
        assertDataFieldEquals(it.next(), "500", ' ', ' ', "a", "Songs.");
        assertDataFieldEquals(it.next(), "511", '0', ' ', "a",
        ...
        assertDataFieldEquals(it.next(), "856", '4', '1', "3", "Preservation copy (limited access)", "u",
    ...
    static void assertControlFieldInRecordEquals(final String tag, final String expected, final VariableField field) {
        ControlField tmp;
        tmp = (ControlField) field;
        assertEquals("Control field mismatch for tag " + tag, expected, tmp.getData());
    }

    static void assertDataFieldEquals(final VariableField vf, final String tag, final char ind1, final char ind2,
            final String... subfieldsAndValues) {
        final DataField field = (DataField) vf;
        ...
        final Iterator<Subfield> it = subfields.iterator();
        for (int i = 0; i < subfieldsAndValues.length; i++) {
            final String expectedCode = subfieldsAndValues[i++];
            final String expectedValue = subfieldsAndValues[i];
            if (!it.hasNext()) {
                fail("not enough subfields - expecting $" + expectedCode + " = " + expectedValue);
            }

            final Subfield sf = it.next();
     ...
     * @throws IOException
     */
    public static void validateBytesAgainstFile(final byte[] actual, final String fileName) throws IOException {
        final InputStream stream = TestUtils.class.getResourceAsStream(fileName);
        ...
        final String comparison = compareFilesContentsLineByLine(new String(expected), new String(actual));
     ...
     * @throws IOException
     */
    public static void validateStringAgainstFile(final String actual, final String fileName) throws IOException {
        final InputStream stream = TestUtils.class.getResourceAsStream(fileName);
        ...
        os.flush();
        final String expected = new String(os.toByteArray());
        final String comparison = compareFilesContentsLineByLine(expected, actual);
        if (comparison != null) {
            fail("actual differs from expected as shown below:" + System.getProperty("line.separator") + comparison);
        }
    }
     ...
     * @return
     */
    public static String compareFilesContentsLineByLine(final String expected, final String actual) {
        ...
        while (expectedIndex < expectedLines.length && actualIndex < actualLines.length) {
            if (expectedLines[expectedIndex].equals(actualLines[actualIndex])) {
                ...
                actualIndex++;
                    ...
                    expectedLines[expectedIndex].equals(actualLines[actualIndex + 1])) {
                sb.append("+ " + actualLines[actualIndex]).append(separator);
                actualIndex++;
                matches = false;
            } else if (expectedIndex + 1 < expectedLines.length &&
                    expectedLines[expectedIndex + 1].equals(actualLines[actualIndex])) {
                sb.append("- " + expectedLines[expectedIndex]).append(separator);
                expectedIndex++;
                matches = false;
            } else {
