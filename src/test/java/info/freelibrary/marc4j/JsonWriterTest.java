...
public class JsonWriterTest {
    ...
    public void testMarcInJsonWriter() throws Exception {
        ...
        MarcJsonWriter writer =
                new MarcJsonWriter(out, MarcJsonWriter.MARC_IN_JSON);
        Record record = getSummerlandRecord();
        writer.write(record);
        TestUtils.validateBytesAgainstFile(out.toByteArray(),
                StaticTestRecords.RESOURCES_SUMMERLAND_MARC_IN_JSON_JSON);
        writer.close();
    }

    /**
     * Tests {@link JsonWriter}'s ability to write indented JSON.
     * 
     * @throws Exception
     */
    @Test
    public void testMarcInJsonWriterIndented() throws Exception {
        ...
        writer.write(record);
        TestUtils
                .validateBytesAgainstFile(
                        out.toByteArray(),
                        StaticTestRecords.RESOURCES_SUMMERLAND_MARC_IN_JSON_INDENTED_JSON);
        writer.close();
    }

    /**
     * Tests {@link MarcJsonWriter}
     * 
     * @throws Exception
     */
    @Test
    public void testMarcJsonWriter() throws Exception {
        ...
        Record record = getSummerlandRecord();
        MarcJsonWriter writer =
                new MarcJsonWriter(out, MarcJsonWriter.MARC_JSON);
        writer.write(record);
        TestUtils.validateBytesAgainstFile(out.toByteArray(),
                StaticTestRecords.RESOURCES_SUMMERLAND_MARC_JSON_JSON);
        writer.close();

    }

    /**
     * Tests {@link MarcJsonWriter} ability to write indented JSON.
     * 
     * @throws Exception
     */
    @Test
    public void testMarcJsonWriterIndented() throws Exception {
        ...
        writer.write(record);
        TestUtils.validateBytesAgainstFile(out.toByteArray(),
                StaticTestRecords.RESOURCES_SUMMERLAND_INDENTED_MARC_JSON_JSON);

        writer.close();
    }

    /**
     * Tests {@link MarcJsonWriter} ability to write to UTF-8 from ANSEL.
     * 
     * @throws Exception
     */
    @Test
    public void testMarcJsonWriterConvertedToUTF8() throws Exception {
        ...
        InputStream input =
                getClass().getResourceAsStream(
                        StaticTestRecords.RESOURCES_BRKRTEST_MRC);
            ...
            if (line.matches("[ ]*\"a\":\"This is a test of diacritics.*")) {
                ...
                for (int i = 0; i < lineParts.length; i++) {
                    if (lineParts[i].startsWith("the tilde in ")) {
                        assertTrue("Incorrect value for tilde", lineParts[i]
                                .equals("the tilde in man\\u0303ana"));
                    } else if (lineParts[i].startsWith("the grave accent in ")) {
                        assertTrue("Incorrect value for grave", lineParts[i]
                                .equals("the grave accent in tre\\u0300s"));
                    } else if (lineParts[i].startsWith("the acute accent in ")) {
                        assertTrue(
                                "Incorrect value for acute",
                                lineParts[i]
                                        .equals("the acute accent in de\\u0301sire\\u0301e"));
                    } else if (lineParts[i].startsWith("the circumflex in ")) {
                        assertTrue("Incorrect value for macron", lineParts[i]
                                .equals("the circumflex in co\\u0302te"));
                    } else if (lineParts[i].startsWith("the macron in ")) {
                        assertTrue("Incorrect value for macron", lineParts[i]
                                .equals("the macron in To\\u0304kyo"));
                    } else if (lineParts[i].startsWith("the breve in ")) {
                        assertTrue("Incorrect value for breve", lineParts[i]
                                .equals("the breve in russkii\\u0306"));
                    } else if (lineParts[i].startsWith("the dot above in ")) {
                        assertTrue("Incorrect value for dot above",
                                lineParts[i]
                                        .equals("the dot above in z\\u0307aba"));
                    } else if (lineParts[i]
                            .startsWith("the dieresis (umlaut) in ")) {
                        assertTrue(
                                "Incorrect value for umlaut",
                                lineParts[i]
                                        .equals("the dieresis (umlaut) in Lo\\u0308wenbra\\u0308u"));
                    }
                }
            }
        }
     ...
     */
    @Test
    public void testMarcJsonWriterConvertedToUTF8AndNormalized()
            throws Exception {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        ...
        while ((line = testoutput.readLine()) != null) {
            if (line.matches("[ ]*\"a\":\"This is a test of diacritics.*")) {
                ...
                for (int i = 0; i < lineParts.length; i++) {
                    if (lineParts[i].startsWith("the tilde in ")) {
                        assertTrue(
                                "Incorrect normalized value for tilde accent",
                                lineParts[i].equals("the tilde in ma\u00f1ana"));
                    } else if (lineParts[i].startsWith("the grave accent in ")) {
                        assertTrue(
                                "Incorrect normalized value for grave accent",
                                lineParts[i]
                                        .equals("the grave accent in tr\u00e8s"));
                    } else if (lineParts[i].startsWith("the acute accent in ")) {
                        assertTrue(
                                "Incorrect normalized value for acute accent",
                                lineParts[i]
                                        .equals("the acute accent in d\u00e9sir\u00E9e"));
                    } else if (lineParts[i].startsWith("the circumflex in ")) {
                        assertTrue("Incorrect normalized value for circumflex",
                                lineParts[i]
                                        .equals("the circumflex in c\u00f4te"));
                    } else if (lineParts[i].startsWith("the macron in ")) {
                        assertTrue("Incorrect normalized value for macron",
                                lineParts[i]
                                        .equals("the macron in T\\u014dkyo"));
                    } else if (lineParts[i].startsWith("the breve in ")) {
                        assertTrue("Incorrect normalized value for breve",
                                lineParts[i]
                                        .equals("the breve in russki\\u012d"));
                    } else if (lineParts[i].startsWith("the dot above in ")) {
                        assertTrue("Incorrect normalized value for dot above",
                                lineParts[i]
    ...
    /**
     * Tests {@link MarcJsonWriter} and {@link MarcJsonReader}.
     * 
     * @throws Exception
     */
    @Test
    public void testJsonWriteAndRead() throws Exception {
        Record record =
                getJSONRecordFromFile(StaticTestRecords.RESOURCES_LEGAL_JSON_MARC_IN_JSON_JSON);
