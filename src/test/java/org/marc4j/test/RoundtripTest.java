...
public class RoundtripTest {
    ...
    /**
     * This test reads in a file of utf-8 encoded binary Marc records then
     * writes those records out as utf-8 encoded binary Marc records, then reads
     * those records. The test then compares those records with the original
    ...
    public void testWriteAndReadRoundtrip() throws Exception {
        ...
        InputStream inputCompare1 =
                getClass().getResourceAsStream(
                        StaticTestRecords.RESOURCES_CHABON_MRC);
        InputStream inputCompare2 =
    ...
    /**
     * This test reads in a file of Marc8 encoded binary Marc records then
     * writes those records out as utf-8 encoded binary Marc records, then reads
     * those records back in and writes them out as Marc8 encoded binary Marc
     * records. The test then compares those records with the original records,
    ...
    /**
     * This test reads in a file of Marc8 encoded binary Marc records then
     * writes those records out as utf-8 encoded MarcXML records, then reads
     * those records back in and writes them out as Marc8 binary Marc records.
     * The test then compares those binary Marc records with the original
     * records, expecting them to be identical.
     * 
     * @throws Exception
     */
    ...
    /**
     * This test reads in a file of Marc8 encoded binary Marc records then
     * writes those records out as utf-8 encoded MarcXML records using unicode
     * normalization, which combines diacritics with the character they adorn
     * (whenever possible). It then reads those records back in and writes them
     * out as Marc8 binary Marc records. The test then compares those binary
     * Marc records with original binary Marc records, expecting them to be
     * identical. Note: Since there are multiple ways of representing some
     * unicode characters in marc8, it is not possible to guarantee
     * roundtripping from marc8 encoded records to utf-8 and back to marc8, the
     * likelihood is even higher when the utf-8 characters are normalized. It is
     * possible to guarantee roundtripping from normalized utf-8 encoded records
     * to marc8 encoded binary marc records back to normalized utf-8 records.
     * 
     * @throws Exception
     */
    @Test
