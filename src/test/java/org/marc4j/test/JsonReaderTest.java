...
public class JsonReaderTest {
    ...
    public void testMarcJsonReaders() throws Exception {
        InputStream input1 =
                getResourceAsStream(StaticTestRecords.RESOURCES_MARC_JSON_JSON);
        MarcReader reader1 = new MarcJsonReader(input1);

        InputStream input2 =
                getResourceAsStream(StaticTestRecords.RESOURCES_MARC_IN_JSON_JSON);
        ...
        while (reader1.hasNext() && reader2.hasNext()) {
            ...
            for (int i = 0; i < recordAsStrings1.length; i++) {
                assertEquals("line mismatch between two records",
                        recordAsStrings1[i], recordAsStrings2[i]);
            }
