...
public class MarcXmlWriterTest {
    ...
    public void testMarcXmlWriter() throws Exception {
        ...
        writer.close();
        TestUtils.validateStringAgainstFile(new String(out.toByteArray()),
                StaticTestRecords.RESOURCES_SUMMERLAND_XML);
    }
            ...
            if (line.matches("[ ]*<subfield code=\"a\">This is a test of diacritics.*")) {
                ...
                for (String linePart : lineParts) {
                    if (linePart.startsWith("the tilde in ")) {
                        assertTrue("Incorrect value for tilde", linePart
                                .equals("the tilde in man\u0303ana"));
                    } else if (linePart.startsWith("the grave accent in ")) {
                        assertTrue("Incorrect value for grave", linePart
                                .equals("the grave accent in tre\u0300s"));
                    } else if (linePart.startsWith("the acute accent in ")) {
                        assertTrue(
                                "Incorrect value for acute",
                                linePart.equals("the acute accent in de\u0301sire\u0301e"));
                    } else if (linePart.startsWith("the circumflex in ")) {
                        assertTrue("Incorrect value for macron", linePart
                                .equals("the circumflex in co\u0302te"));
                    } else if (linePart.startsWith("the macron in ")) {
                        assertTrue("Incorrect value for macron", linePart
                                .equals("the macron in To\u0304kyo"));
                    } else if (linePart.startsWith("the breve in ")) {
                        assertTrue("Incorrect value for breve", linePart
                                .equals("the breve in russkii\u0306"));
                    } else if (linePart.startsWith("the dot above in ")) {
                        assertTrue("Incorrect value for dot above", linePart
                                .equals("the dot above in z\u0307aba"));
                    } else if (linePart.startsWith("the dieresis (umlaut) in ")) {
                        assertTrue(
                                "Incorrect value for umlaut",
                                linePart.equals("the dieresis (umlaut) in Lo\u0308wenbra\u0308u"));
                    }
                }
            }
        }
    ...
    public void testWriteOfRecordWithIndicatorlessSubfield() throws Exception {
        ...
        badField.setTag("911");
        badField.addSubfield(factory.newSubfield('a',
                ...
                for (int i = 0; i < lineParts.length; i++) {
                    if (lineParts[i].startsWith("the tilde in ")) {
                        ...
                        assertTrue("Incorrect normalized value for macron",
                                lineParts[i].equals("the macron in T\u014Dkyo"));
