...
public class MarcCombiningReader implements MarcReader {
     ...
     * Constructor for a "combining" Marc reader, that looks ahead at the Marc
     * file to determine when the next record is a continuation of the currently
     * read record. Because this reader needs to have two records in memory to
     * determine when the subsequent record is a continuation, if Error Handling
     * is being performed, this constructor needs to be used, so that the errors
     * from the "next" record are not appended to the results for the "current"
     * record. Call this constructor in the following way: ErrorHandler errors2
     * = errors; errors = new ErrorHandler(); reader = new
     * MarcCombiningReader(reader, errors, errors2,
     * combineConsecutiveRecordsFields);
     * 
     * @param reader - The Lower level MarcReader that returns Marc4J Record
     *        objects that are read from a Marc file.
        ...
        if (currentRecord != null) {
            ...
            try {
                ...
                if (currentRecord != null) {
                    ...
                    throw new MarcException("Marc record couldn't be read -- " +
                            e.toString());
                }
            }
            ...
            while (recordsMatch(currentRecord, nextRecord)) {
                currentRecord =
                        combineRecords(currentRecord, nextRecord, idsToMerge);
