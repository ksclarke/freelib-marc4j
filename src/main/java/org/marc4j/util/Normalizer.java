...
public class Normalizer {
    ...
    public static String normalize(String str, int mode) {
        ...
        try {
            if (hasJavaTextNormalizer) {
                if (mode == NFD) {
                    result =
                            normalizeMethod.invoke(null, str, NFD_obj)
                                    .toString();
                } else if (mode == NFC) {
                    result =
                            normalizeMethod.invoke(null, str, NFC_obj)
                                    .toString();
                } else if (mode == NFKD) {
                    result =
                            normalizeMethod.invoke(null, str, NFKD_obj)
                                    .toString();
                } else if (mode == NFKC) {
                    result =
                            normalizeMethod.invoke(null, str, NFKC_obj)
                                    .toString();
                } else {
                    result = str;
                }
                ...
                if (mode == NFD) {
                    result =
                            decomposeMethod.invoke(null, str, false).toString();
                } else if (mode == NFC) {
    ...
    private static void initializeNormalizer() {
        try {
            ...
            try {
                composeMethod =
                        normalizerInAvailableJar.getMethod("compose",
