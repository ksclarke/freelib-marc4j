...
public class AnselToUnicode extends CharConverter {
    ...
    /**
     * Returns <code>true</code> if NCR should be translated; else,
     * <code>false</code>.
     * 
    ...
    /**
     * Creates a new instance and loads the MARC4J supplied conversion tables
     * based on the official LC tables.
     */
    ...
    private CodeTableInterface loadGeneratedTable(boolean loadMultibyte) {
        try {
            Class<?> generated =
                    Class.forName("org.marc4j.converter.impl.CodeTableGenerated");
    ...
    /**
     * Loads the entire mapping (including multibyte characters) from the
     * Library of Congress.
     */
    private void loadMultibyte() {
        ct =
                new CodeTable(getClass().getResourceAsStream(
    ...
    private void checkMode(char[] data, CodeTracker cdt) {
        int extra = 0;
        int extra2 = 0;
        while (cdt.offset + extra + extra2 < data.length &&
                isEscape(data[cdt.offset])) {
            if (cdt.offset + extra + extra2 + 1 == data.length) {
                cdt.offset += 1;
                if (errorList != null) {
                    errorList
                            .addError(ErrorHandler.MINOR_ERROR,
                                    "Escape character found at end of field, discarding it.");
                } else {
                    throw new MarcException(
                            "Escape character found at end of field");
                }
                break;
            }
        ...
        if (errorList != null && (extra != 0 || extra2 != 0)) {
            errorList
                    .addError(ErrorHandler.ERROR_TYPO, "" + (extra + extra2) +
                            " extraneous space characters found within MARC8 character set escape sequence");
        }
    }

    private void set_cdt(CodeTracker cdt, int g0_or_g1, char[] data,
            int addnlOffset, boolean multibyte) {
        if (data[cdt.offset + addnlOffset] == '!' &&
                data[cdt.offset + addnlOffset + 1] == 'E') {
            addnlOffset++;
    ...
    public String convert(char data[]) {
            ...
            if (ct.isCombining(data[cdt.offset], cdt.g0, cdt.g1) &&
                ...
                if (data[cdt.offset] == 0x20) {
                    ...
                    cdt.offset += 1;
                        ...
                        (errorList == null || data[cdt.offset + 1] != 0x20 &&
                                data[cdt.offset + 2] != 0x20)) {
                    char c =
                            getMBChar(makeMultibyte(data[cdt.offset],
                                    data[cdt.offset + 1], data[cdt.offset + 2]));
                    if (errorList == null || c != 0) {
                        sb.append(c);
                        cdt.offset += 3;
                    } else if (cdt.offset + 6 <= data.length &&
                            data[cdt.offset + 4] != 0x20 &&
                            data[cdt.offset + 5] != 0x20 &&
                            getMBChar(makeMultibyte(data[cdt.offset + 3],
                                    data[cdt.offset + 4], data[cdt.offset + 5])) != 0) {
                        if (errorList != null) {
                            errorList
                                    .addError(
                                            ErrorHandler.MINOR_ERROR,
                    ...
                    } else if (cdt.offset + 4 <= data.length &&
                            data[cdt.offset] > 0x7f &&
                            getMBChar(makeMultibyte(data[cdt.offset + 1],
                                    data[cdt.offset + 2], data[cdt.offset + 3])) != 0) {
                        if (errorList != null) {
                            errorList
                                    .addError(
                                            ErrorHandler.MINOR_ERROR,
                        ...
                        cdt.offset + 4 <= data.length &&
                        (data[cdt.offset + 1] == 0x20 || data[cdt.offset + 2] == 0x20)) {
                    int multiByte =
                            makeMultibyte(data[cdt.offset],
                                    ((data[cdt.offset + 1] != 0x20)
                                            ? data[cdt.offset + 1]
                                            : data[cdt.offset + 2]),
                                    data[cdt.offset + 3]);
                    ...
                    if (c != 0) {
                        if (errorList != null) {
                            errorList
                                    .addError(ErrorHandler.ERROR_TYPO,
                ...
                if (c != 0) {
                    ...
                    String val =
                            "0000" +
                                    Integer.toHexString((int) (data[cdt.offset]));
                    sb.append("<U+" +
        ...
        if (translateNCR &&
            ...
            Pattern pattern =
                    Pattern.compile("&#x([0-9A-Fa-f][0-9A-Fa-f][0-9A-Fa-f][0-9A-Fa-f]);");
