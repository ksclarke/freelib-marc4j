...
public class UnicodeToAnsel extends CharConverter {
     ...
     * conversion tables based on the official LC tables. Loads in the generated
     * class ReverseCodeTableGenerated which contains switch statements to
     * lookup the MARC-8 encodings for given Unicode characters.
     */
    ...
    /**
     * Constructs an instance with the specified pathname. Use this constructor
     * to create an instance with a customized code table mapping. The mapping
     * file should follow the structure of LC's XML MARC-8 to Unicode mapping
    ...
    /**
     * Constructs an instance with the specified input stream. Use this
     * constructor to create an instance with a customized code table mapping.
     * The mapping file should follow the structure of LC's XML MARC-8 to
     * Unicode mapping (see:
    ...
    private ReverseCodeTable loadGeneratedTable() {
        try {
            ...
            rct =
                    new ReverseCodeTableHash(AnselToUnicode.class
                            .getResourceAsStream("resources/codetables.xml"));
            return (rct);
        }
    }
     ...
     * <p>
     * If there is no match for a Unicode character, it will be encoded as
     * &#xXXXX; so that if the data is translated back into Unicode, the
     * original data can be recreated.
     * </p>
     * 
     ...
     * <p>
     * If the Unicode data has been normalized into composed form, and the
     * composed character does not have a corresponding MARC8 character, this
     * routine will normalize that character into its decomposed form, and try
     * to translate that equivalent string into MARC8.
     * </p>
     * 
     * @param data - the UCS/Unicode data in an array of char
     * @return String - the MARC-8 data
    ...
    private void convertPortion(char data[], StringBuffer sb) {
        for (int i = 0; i < data.length; i++) {
            ...
            if (charValue == 0x20 && rct.getPreviousG0() != (int) '1') {
                ...
                // Unicode character c has no match in the Marc8 tables. Try
                // unicode-decompose on it
                // to see whether the decomposed form can be represented. If
                // when decomposed, all of
                // the characters can be translated to marc8, then use that. If
                // not and the decomposed form
                // if three (or more) characters long (which indicates multiple
                // diacritic marks), then
                // re-compose the the main character with the first diacritic,
                // and check whether that
                // and the remaining diacritics can be translated. If so go with
                // that, otherwise, give up
                // and merely use the &#xXXXX; Numeric Character Reference form
                // to represent the original
                // unicode character
                String tmpnorm = c.toString();
                String tmpNormed =
                        Normalizer.normalize(tmpnorm, Normalizer.NFD);
                if (!tmpNormed.equals(tmpnorm)) {
                    if (allCharsHaveMatch(rct, tmpNormed)) {
                        ...
                        String partialNormed =
                                Normalizer.normalize(firstTwo, Normalizer.NFC);
                        if (!partialNormed.equals(firstTwo) &&
                                allCharsHaveMatch(rct, partialNormed) &&
                                allCharsHaveMatch(rct, tmpNormed.substring(2))) {
                            convertPortion((partialNormed + tmpNormed
                                    .substring(2)).toCharArray(), sb);
                            continue;
                        }
                    }
                }
                ...
                if (charValue < 0x1000) {
                    sb.append("&#x" +
                            Integer.toHexString(charValue + 0x10000)
                ...
                // if several MARC-8 character sets contain the given Unicode
                // character, select the
                // best char set to use for encoding the character. Preference
                // is given to character
                // sets that have been used previously in the field being
                // encoded. Since the default
                // character sets for Basic and extended latin are pre-loaded,
            ...
            if (rct.isCombining(c) && sb.length() > 0) {
                ...
                // Special case handling to handle the COMBINING DOUBLE INVERTED
                // BREVE
                // and the COMBINING DOUBLE TILDE where a single double wide
                // accent character
